...
class ProfileIncomeFragment : BaseFragment<MainActivity>(), ProfileIncomeContract.View,
    ...
    override fun initializeViews() {
        ...
        val testList: MutableList<IncomesItem> = mutableListOf()

        testList.add(
            IncomeDateItem(data = "30 December")
        )
        testList.add(
            IncomeListItem(
                data = IncomeModel(id = 1, date = "26 July - 1 August", price = 100)
            )
        )
        testList.add(
            IncomeListItem(
                data = IncomeModel(id = 1, date = "2 August - 8 August", price = 15000)
            )
        )
        testList.add(
            IncomeDateItem(data = "30 December")
