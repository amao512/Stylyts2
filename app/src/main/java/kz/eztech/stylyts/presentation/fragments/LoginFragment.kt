...
class LoginFragment : BaseFragment<AuthorizationActivity>(),LoginContract.View, View.OnClickListener {
    @Inject
    ...
    override fun onClick(v: View?) {
        when(v?.id){
            ...
            R.id.text_view_fragment_login_forgot_password -> {
                hideSoftWareKeyboard()
                view?.findNavController()?.navigate(R.id.action_loginFragment_to_refreshPasswordFragment)
            }
    ...
    override fun processLoginUser(userModel: UserModel) {
        currentActivity.saveSharedPrefByKey(SharedConstants.TOKEN_KEY,userModel.token)
        currentActivity.saveSharedPrefByKey(SharedConstants.USER_ID_KEY,userModel.id)
