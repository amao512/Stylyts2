...
class ItemDetailFragment : BaseFragment<MainActivity>(), ItemDetailContract.View,View.OnClickListener,
	...
	override fun initializeViews() {
		currentClotheModel?.let {
			...
			text_view_fragment_item_detail_brand_name.text = "${it.brand?.first_name} ${it.brand?.last_name}"
			text_view_fragment_item_detail_item_name.text = it.title
			text_view_fragment_item_detail_item_price.text = "${NumberFormat.getInstance().format(it.cost)} ${it.currency}"
		...
		when(v?.id){
			...
			R.id.linear_layout_fragment_item_detail_description_holder -> {
				if (text_view_fragment_item_detail_description.visibility == View.VISIBLE) {
					...
					val objectAnimator = ObjectAnimator.ofFloat(image_view_fragment_item_detail_description_arrow, View.ROTATION,
